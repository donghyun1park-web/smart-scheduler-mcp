...
def calibrate_project_completion(project_path: str | Path, patch: CalibrationPatch) -> dict[str, object]:
    ...
    if not isinstance(project, Project):
        return {"ok": False, "warnings": ["Project not found"], "comparison": {}}
    calendar_model = db.get_calendar(project_path, project.calendar_id)
    if calendar_model is None:
        return {"ok": False, "warnings": ["Project calendar not found"], "comparison": {}}

    warnings = _validate_patch(activities, relationships, patch)
    ...
    if after_cpm.cycles_detected:
        warnings.append(f"Calibration creates dependency cycle: {after_cpm.cycles_detected}")
...
def _validate_patch(
    ...
    for dependency_override in patch.dependency_overrides or []:
        if dependency_override.predecessor_id not in activity_ids:
            warnings.append(f"Unknown predecessor_id: {dependency_override.predecessor_id}")
        if dependency_override.successor_id not in activity_ids:
            warnings.append(f"Unknown successor_id: {dependency_override.successor_id}")
        if dependency_override.dependency_type not in {"FS", "SS", "FF"}:
            warnings.append(f"Unsupported dependency_type for v0.1: {dependency_override.dependency_type}")
    for duration_override in patch.duration_overrides or []:
        if duration_override.task_id not in activity_ids:
            warnings.append(f"Unknown task_id: {duration_override.task_id}")
        if duration_override.duration_days < 0:
            warnings.append(f"Invalid duration_days for {duration_override.task_id}: {duration_override.duration_days}")
    for lag_override in patch.lag_overrides or []:
        if lag_override.predecessor_id not in activity_ids:
            warnings.append(f"Unknown predecessor_id: {lag_override.predecessor_id}")
        if lag_override.successor_id not in activity_ids:
            warnings.append(f"Unknown successor_id: {lag_override.successor_id}")
        if (lag_override.predecessor_id, lag_override.successor_id) not in relationship_keys:
            warnings.append(
                "Unknown relationship for lag override: "
                f"{lag_override.predecessor_id}->{lag_override.successor_id}"
            )
    return warnings
